...
class TeamsFeatures:
    ...
    def create_betting_features(self):
        """
        ...
        try:
            ...
            # Características para Win (predicción binaria)
            new_features['win_probability'] = self.teams_data.apply(
                lambda row: row['win_rate_10'] if 'win_rate_10' in self.teams_data.columns else 0.5,
                axis=1
            )
            
            # Características para Total Points (over/under)
            for threshold in self.betting_lines['Total_Points_Over_Under']:
                # Calcular la probabilidad histórica de superar este umbral
                new_features[f'total_points_over_{threshold}'] = (self.teams_data['total_points'] > threshold).astype(int)
                
                # Para cada ventana, calcular probabilidad móvil
                for window in self.window_sizes:
                    if f'total_points_mean_{window}' in self.teams_data.columns:
                        # Crear un DataFrame temporal para los cálculos
                        temp_df = pd.DataFrame({
                            'Team': self.teams_data['Team'],
                            'mean': self.teams_data[f'total_points_mean_{window}']
                        })
                        ...
                        new_features[f'total_points_line_diff_{threshold}_{window}'] = temp_df['line_diff'].values
                        
                        # Probabilidad de superar la línea
                        new_features[f'prob_total_over_{threshold}_{window}'] = (temp_df['line_diff'] > 0).astype(float).values
                        
                        # Calcular estadísticas por equipo
                        grouped = temp_df.groupby('Team')['line_diff']
                        ...
                        try:
                            ...
                            volatility = grouped.transform(
                                lambda x: pd.to_numeric(x, errors='coerce')
                                      ...
                                      .replace([np.inf, -np.inf], np.nan)
                                      .fillna(0)
                                      .clip(0, 30)  # Limitar a rango razonable
                            )
                            new_features[f'total_points_line_volatility_{threshold}_{window}'] = volatility.values
                        except Exception as e:
                            logger.error(f"Error al calcular total_points_line_volatility_{threshold}_{window}: {str(e)}")
                            # Valor por defecto en caso de error
                            new_features[f'total_points_line_volatility_{threshold}_{window}'] = np.zeros(len(self.teams_data))
                        
                        # Tendencia con manejo robusto de valores
                        try:
                            # Convertir a numérico y calcular medias de forma segura
                            recent_mean = grouped.transform(
                                lambda x: pd.to_numeric(x, errors='coerce')
                                      .replace([np.inf, -np.inf], np.nan)
                                      .fillna(0)
                                      .rolling(window=window, min_periods=1)
                            ...
                            older_mean = grouped.transform(
                                lambda x: pd.to_numeric(x, errors='coerce')
                                      .replace([np.inf, -np.inf], np.nan)
                                      .fillna(0)
                                      .rolling(window=window*2, min_periods=1)
                                      .mean()
                            )
                            
                            # Calcular diferencia y manejar valores problemáticos
                            trend = (recent_mean - older_mean)
                            trend = trend.replace([np.inf, -np.inf], np.nan).fillna(0).clip(-30, 30)
                            
                            new_features[f'total_points_line_trend_{threshold}_{window}'] = trend.values
                        except Exception as e:
                            logger.error(f"Error al calcular total_points_line_trend_{threshold}_{window}: {str(e)}")
                            # Valor por defecto en caso de error
                            new_features[f'total_points_line_trend_{threshold}_{window}'] = np.zeros(len(self.teams_data))
            
            # Características para Team Points (over/under)
            for threshold in self.betting_lines['Team_Points_Over_Under']:
                # Calcular la probabilidad histórica de superar este umbral
                new_features[f'team_points_over_{threshold}'] = (self.teams_data['PTS'] > threshold).astype(int)
                
                # Para cada ventana, calcular probabilidad móvil
                for window in self.window_sizes:
                    if f'PTS_mean_{window}' in self.teams_data.columns:
                        # Crear un DataFrame temporal para los cálculos
                        temp_df = pd.DataFrame({
                            'Team': self.teams_data['Team'],
                            'mean': self.teams_data[f'PTS_mean_{window}']
                        })
                        ...
                        # Guardar la diferencia
                        new_features[f'team_points_line_diff_{threshold}_{window}'] = temp_df['line_diff'].values
                        
                        # Probabilidad de superar la línea
                        new_features[f'prob_team_over_{threshold}_{window}'] = (temp_df['line_diff'] > 0).astype(float).values
                        ...
                        try:
                            ...
                            new_features[f'team_points_line_volatility_{threshold}_{window}'] = volatility.values
                        except Exception as e:
                            logger.error(f"Error al calcular total_points_line_volatility_{threshold}_{window}: {str(e)}")
                            # Valor por defecto en caso de error
                            new_features[f'team_points_line_volatility_{threshold}_{window}'] = np.zeros(len(self.teams_data))
                        ...
                        try:
                            ...
                            new_features[f'team_points_line_trend_{threshold}_{window}'] = trend.values
                        except Exception as e:
                            logger.error(f"Error al calcular team_points_line_trend_{threshold}_{window}: {str(e)}")
                            # Valor por defecto en caso de error
                            new_features[f'team_points_line_trend_{threshold}_{window}'] = np.zeros(len(self.teams_data))
            
            # Verificar que todas las características tienen la misma longitud
            lengths = {k: len(v) for k, v in new_features.items()}
            if len(set(lengths.values())) > 1:
                logger.error(f"Longitudes inconsistentes en las características: {lengths}")
                raise ValueError("Las características tienen longitudes diferentes")
            
            # Crear DataFrame con todas las nuevas características
            features_df = pd.DataFrame(new_features, index=self.teams_data.index)
            
            # Concatenar las nuevas características con el DataFrame original
            self.teams_data = pd.concat([self.teams_data, features_df], axis=1)
            
            # Llamar a características avanzadas de apuestas
            self._create_advanced_betting_features()
            
            logger.info("Características para líneas de apuestas creadas")
            return self.teams_data
            
        except Exception as e:
            logger.error(f"Error al crear características de apuestas: {str(e)}")
            raise
    
    def _create_advanced_betting_features(self):
        """
        Crea características avanzadas específicas para líneas de apuestas
        """
        logger.info("Creando características avanzadas para líneas de apuestas")
        
        try:
            # Asegurar que tenemos un índice numérico
            self.teams_data = self.teams_data.reset_index(drop=True)
            
            # Solución alternativa: procesar cada equipo y crear un nuevo DataFrame desde cero
            teams = self.teams_data['Team'].unique()
            logger.debug(f"Procesando características avanzadas para {len(teams)} equipos")
            
            # Columnas base que siempre queremos mantener
            base_cols = ['Team', 'Date', 'Opp', 'PTS', 'PTS_Opp', 'is_win', 'is_home']
            base_cols = [col for col in base_cols if col in self.teams_data.columns]
            
            # Crear un nuevo DataFrame desde cero
            all_teams_data = []
            
            for team in teams:
                logger.debug(f"Procesando equipo: {team}")
                ...
                team_base = team_data[base_cols].copy()
                
                # Crear características nuevas
                new_features = {}
                
                # Consistencia para líneas de total de puntos
                for threshold in self.betting_lines['Total_Points_Over_Under']:
                    over_col = f'total_points_over_{threshold}'
                    if over_col in team_data.columns:
                        try:
                            # Convertir a numérico de forma segura
                            over_values = pd.to_numeric(team_data[over_col], errors='coerce')
                            over_values = over_values.replace([np.inf, -np.inf], np.nan).fillna(0)
                            
                            # Calcular desviación estándar de forma segura
                            over_std = self._safe_rolling(over_values, 10, 'std')
                            
                            # Calcular consistencia - limitando a [0,1] porque es una medida de consistencia
                            consistency = 1 - over_std.clip(0, 1)
                            new_features[f'total_points_line_consistency_{threshold}'] = consistency.clip(0, 1)
                            
                            # Racha con manejo seguro de valores
                            streaks = []
                            current_streak = 0
                            
                            # Asegurar que over_values son enteros 0/1
                            over_binary = over_values.round().astype(int).clip(0, 1)
                            
                            for over in over_binary:
                                if over == 1:  # Usar comparación exacta con entero
                                    current_streak += 1
                                else:
                                    current_streak = 0
                                streaks.append(current_streak)
                                
                            new_features[f'total_points_line_streak_{threshold}'] = streaks
                        except Exception as e:
                            logger.error(f"Error al calcular características para total_points_line_{threshold}: {str(e)}")
                            # Valores por defecto en caso de error
                            new_features[f'total_points_line_consistency_{threshold}'] = pd.Series(0.5, index=team_data.index)
                            new_features[f'total_points_line_streak_{threshold}'] = pd.Series(0, index=team_data.index)
                
                # Consistencia para líneas de puntos por equipo
                for threshold in self.betting_lines['Team_Points_Over_Under']:
                    over_col = f'team_points_over_{threshold}'
                    if over_col in team_data.columns:
                        try:
                            # Convertir a numérico de forma segura
                            over_values = pd.to_numeric(team_data[over_col], errors='coerce')
                            ...
                            # Calcular consistencia - limitando a [0,1] porque es una medida de consistencia
                            consistency = 1 - over_std.clip(0, 1)
                            new_features[f'team_points_line_consistency_{threshold}'] = consistency.clip(0, 1)
                            ...
                            new_features[f'team_points_line_streak_{threshold}'] = streaks
                        except Exception as e:
                            logger.error(f"Error al calcular características para team_points_line_{threshold}: {str(e)}")
                            # Valores por defecto en caso de error
                            new_features[f'team_points_line_consistency_{threshold}'] = pd.Series(0.5, index=team_data.index)
                            new_features[f'team_points_line_streak_{threshold}'] = pd.Series(0, index=team_data.index)
                
                # Momentum para líneas
                for window in self.window_sizes:
                    # Total de puntos
                    if 'total_points' in team_data.columns:
                        pts_mean = self._safe_rolling(team_data['total_points'], window, 'mean')
                        new_features[f'total_points_line_momentum_{window}'] = (team_data['total_points'] > pts_mean).astype(int)
                    
                    # Puntos por equipo
                    if 'PTS' in team_data.columns:
                        pts_team_mean = self._safe_rolling(team_data['PTS'], window, 'mean')
                        new_features[f'team_points_line_momentum_{window}'] = (team_data['PTS'] > pts_team_mean).astype(int)
                
                # Crear DataFrame con las nuevas características
                if new_features:
                    features_df = pd.DataFrame(new_features, index=team_base.index)
                    
                    # Verificar longitudes
                    if len(features_df) != len(team_base):
                        logger.error(f"Error de longitud para equipo {team}: {len(features_df)} vs {len(team_base)}")
                        continue
                    
                    # Concatenar con datos base
                    team_result = pd.concat([team_base.reset_index(drop=True), 
            ...
            if all_teams_data:
                # Concatenar todos los equipos
                result_df = pd.concat(all_teams_data, ignore_index=True)
                logger.debug(f"DataFrame combinado creado. Forma: {result_df.shape}")
                
                # Unir con el DataFrame original para mantener otras columnas
                # Primero, identificar columnas comunes
                common_cols = [col for col in result_df.columns if col in self.teams_data.columns]
                
                if common_cols:
                    # Usar las columnas comunes como clave para el merge
                    merge_keys = ['Team', 'Date']
                    merge_keys = [col for col in merge_keys if col in common_cols]
                    
                    if merge_keys:
                        logger.debug(f"Realizando merge usando claves: {merge_keys}")
                        # Columnas únicas en cada DataFrame
                        result_only_cols = [col for col in result_df.columns if col not in self.teams_data.columns]
                        
                        if result_only_cols:
                            # Realizar merge solo con las columnas nuevas
                            self.teams_data = self.teams_data.merge(
                                result_df[merge_keys + result_only_cols],
                                on=merge_keys,
                                how='left'
                            )
                            logger.debug(f"Merge completado. Forma final: {self.teams_data.shape}")
                        else:
                            logger.debug("No hay columnas únicas para agregar")
                    else:
                        logger.error("No hay claves comunes para el merge")
                        # Usar el resultado como está
                        self.teams_data = result_df
                else:
                    logger.error("No hay columnas comunes entre los DataFrames")
                    # Usar el resultado como está
                    self.teams_data = result_df
            
            logger.info("Características avanzadas para líneas de apuestas creadas")
            
        except Exception as e:
            logger.error(f"Error al crear características avanzadas para líneas de apuestas: {str(e)}")
            logger.error(f"Traza de error: {traceback.format_exc()}")
            # No lanzar la excepción, continuar con el DataFrame actual
    ...
    def generate_features(self, save_path=None):
        ...
        try:
            ...
            # Asegurar que win_probability existe
            # IMPORTANTE: Crear win_probability de forma robusta
            logger.debug("Creando win_probability de forma robusta")
            try:
                if 'win_rate_10' in self.teams_data.columns:
                    logger.debug("Usando win_rate_10 para win_probability")
                    
                    # Primero, arreglar valores problemáticos en win_rate_10
                    win_rate_10 = pd.to_numeric(self.teams_data['win_rate_10'], errors='coerce')
                    
                    # Reemplazar valores problemáticos
                    win_rate_10 = win_rate_10.replace([np.inf, -np.inf], np.nan)
                    
                    # Procesar equipo por equipo para mejorar precisión
                    win_probs = {}
                    for team in self.teams_data['Team'].unique():
                        team_mask = self.teams_data['Team'] == team
                        team_win_rate = win_rate_10.loc[team_mask]
                        
                        # Si hay NaN en este equipo, rellenar con el promedio del equipo o 0.5
                        if team_win_rate.isnull().any():
                            valid_rates = team_win_rate[~team_win_rate.isnull()]
                            if len(valid_rates) > 0:
                                fill_value = valid_rates.mean()
                            else:
                                # Si no hay valores válidos para este equipo, usar historial general
                                if 'is_win' in self.teams_data.columns:
                                    team_history = self.teams_data.loc[team_mask, 'is_win']
                                    if len(team_history) > 0:
                                        fill_value = team_history.mean()
                                    else:
                                        fill_value = 0.5
                                else:
                                    fill_value = 0.5
                            
                            # Rellenar NaN para este equipo
                            team_win_rate = team_win_rate.fillna(fill_value)
                        
                        # Limitar al rango [0,1]
                        team_win_rate = team_win_rate.clip(0, 1)
                        
                        # Guardar para este equipo
                        for idx in self.teams_data.loc[team_mask].index:
                            win_probs[idx] = team_win_rate.loc[idx]
                    
                    # Convertir el diccionario a Series y asignar al DataFrame
                    if win_probs:
                        self.teams_data['win_probability'] = pd.Series(win_probs)
                        # Asegurar que no hay valores nulos
                        self.teams_data['win_probability'] = self.teams_data['win_probability'].fillna(0.5)
                    else:
                        # Fallback al método original
                        win_rate = pd.to_numeric(self.teams_data['win_rate_10'], errors='coerce')
                        win_rate = win_rate.replace([np.inf, -np.inf], np.nan).fillna(0.5)
                        self.teams_data['win_probability'] = win_rate.clip(0, 1)
                    
                elif 'net_rating' in self.teams_data.columns:
                    logger.debug("Calculando win_probability a partir de net_rating")
                    # Convertir a numérico de forma segura
                    net_rating = pd.to_numeric(self.teams_data['net_rating'], errors='coerce')
                    
                    # Reemplazar infinitos y valores extremos
                    net_rating = net_rating.replace([np.inf, -np.inf], np.nan).fillna(0)
                    
                    # Limitar net_rating para evitar desbordamientos
                    net_rating = net_rating.clip(-20, 20)
                    
                    # Aplicar transformación sigmoide más robusta
                    # Usar parámetro más estable para una transición más suave
                    self.teams_data['win_probability'] = 1 / (1 + np.exp(-net_rating / 10))
                    
                    # Verificar que win_probability está en rango [0,1]
                    self.teams_data['win_probability'] = self.teams_data['win_probability'].clip(0, 1).fillna(0.5)
                    
                    # Ajustar valores extremos para evitar sobreconfianza
                    self.teams_data['win_probability'] = 0.05 + 0.9 * self.teams_data['win_probability']
                    
                elif 'is_win' in self.teams_data.columns:
                    logger.debug("Calculando win_probability a partir de is_win")
                    # Convertir is_win a 0/1 para asegurar cálculo correcto
                    is_win = pd.to_numeric(self.teams_data['is_win'], errors='coerce')
                    is_win = is_win.replace([np.inf, -np.inf], np.nan).fillna(0)
                    is_win = is_win.clip(0, 1)  # Asegurar valores binarios
                    
                    # Crear una versión basada en el historial de victorias por equipo
                    # con memoria exponencial para dar más peso a resultados recientes
                    win_probs = {}
                    
                    for team in self.teams_data['Team'].unique():
                        team_mask = self.teams_data['Team'] == team
                        team_data = self.teams_data.loc[team_mask].sort_values('Date')
                        team_wins = pd.to_numeric(team_data['is_win'], errors='coerce').fillna(0).clip(0, 1)
                        
                        if len(team_wins) > 0:
                            # Calcular EMA de victorias con peso decreciente en partidos anteriores
                            alpha = 0.15  # Factor de memoria para EMA
                            ema_wins = team_wins.ewm(alpha=alpha, adjust=False).mean()
                            
                            # Corregir arranque lento de EMA con valores iniciales
                            if len(ema_wins) >= 10:
                                # Para las primeras 10 posiciones, usar una transición suave
                                for i in range(min(10, len(ema_wins))):
                                    # Mezclar media simple y EMA al inicio
                                    weight = i / 10.0  # De 0 a 1
                                    simple_mean = team_wins[:i+1].mean() if i > 0 else 0.5
                                    ema_wins.iloc[i] = weight * ema_wins.iloc[i] + (1-weight) * simple_mean
                            
                            # Guardar valores para este equipo
                            for i, idx in enumerate(team_data.index):
                                win_probs[idx] = ema_wins.iloc[i]
                        else:
                            # Si no hay datos, asignar valor por defecto
                    ...
                    if win_probs:
                        ...
                        # Fallback al método original si hay problemas
                        self.teams_data['win_probability'] = self.teams_data.groupby('Team')['is_win'].transform(
                            lambda x: pd.to_numeric(x, errors='coerce')
                                  .replace([np.inf, -np.inf], np.nan)
                                  .fillna(0)
                                  .clip(0, 1)
                    ...
                    # Limitar al rango [0.05, 0.95] para evitar extremos
                    self.teams_data['win_probability'] = self.teams_data['win_probability'].clip(0.05, 0.95)
                
                else:
                    logger.debug("Usando valor por defecto para win_probability")
                    # Usar un valor por defecto si no hay métricas disponibles
                    self.teams_data['win_probability'] = 0.5
                
                # Verificar que win_probability se creó correctamente
                if 'win_probability' not in self.teams_data.columns:
                    logger.error("win_probability no se creó correctamente")
                    self.teams_data['win_probability'] = 0.5
                elif self.teams_data['win_probability'].isnull().any():
                    logger.warning("win_probability contiene valores nulos, rellenando con 0.5")
                    self.teams_data['win_probability'] = self.teams_data['win_probability'].fillna(0.5)
                
                # Verificación final de rango
                self.teams_data['win_probability'] = self.teams_data['win_probability'].clip(0, 1)
                
                # Verificar si hay equipos con valores constantes
                teams_with_constant = []
                for team in self.teams_data['Team'].unique():
                    team_vals = self.teams_data.loc[self.teams_data['Team'] == team, 'win_probability']
                    if team_vals.nunique() == 1:
                        teams_with_constant.append(team)
                
                if teams_with_constant:
                    logger.warning(f"Hay {len(teams_with_constant)} equipos con valor constante en win_probability")
                    # Intentar redistribuir valores de win_probability para esos equipos
                    for team in teams_with_constant:
                        team_mask = self.teams_data['Team'] == team
                        team_size = team_mask.sum()
                        if team_size > 5:  # Solo si hay suficientes registros
                            # Crear variación artificial alrededor del valor constante
                            constant_val = self.teams_data.loc[team_mask, 'win_probability'].iloc[0]
                            # Generar valores que varían ligeramente alrededor del valor constante
                            variation = np.linspace(-0.05, 0.05, team_size)
                            new_values = constant_val + variation
                            # Limitar al rango [0,1]
                            new_values = np.clip(new_values, 0, 1)
                            # Asignar nuevos valores
                            self.teams_data.loc[team_mask, 'win_probability'] = new_values
                
                logger.debug("win_probability creada correctamente")
            except Exception as e:
                logger.error(f"Error al crear win_probability: {str(e)}")
                logger.error(f"Traza de error: {traceback.format_exc()}")
                # Asegurar que win_probability existe incluso si hay error
                self.teams_data['win_probability'] = 0.5
                logger.debug("win_probability creada con valor por defecto debido a error")
